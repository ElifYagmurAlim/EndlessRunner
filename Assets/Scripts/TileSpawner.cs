...
namespace TempleRun
{
    public class TileSpwaner :MonoBehaviour
    {
        [SerializeField]
        ...
        private void Start()
        {
            ...
            {
                SpawnTile(startingTitle.GetComponent<Tile>(), true);
            }
        ...
        public void AddNewDirection(Vector3 direction)
        {
            ...
            {
                tilePlacementScale = Vector3.Scale(prevTile.GetComponent<Renderer>().bounds.size /2 + (Vector3.one * startingTitle.GetComponent<BoxCollider>().size.z / 2),currentTileDirection);   
            }
            else
            {
                tilePlacementScale = Vector3.Scale(prevTile.GetComponent<Renderer>().bounds.size - (Vector3.one * 2) + (Vector3.one * startingTitle.GetComponent<BoxCollider>().size.z / 2), currentTileDirection);
